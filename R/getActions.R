...
getActions <- function(sol) {
  ...
  # #Variable V
  # size_v <- length(stringr::str_subset(model$n_variable, "v"))
  # indexes_v <- which(!is.na(stringr::str_extract(model$n_variable, "v")))
  # var_v <- data.frame(id = indexes_v,
  #                     i = rep(0, size_v),
  #                     k = rep(0, size_v),
  #                     t = rep(0, size_v),
  #                     n = rep(0, size_v),
  #                     sol = round(solution$x[indexes_v[1]:indexes_v[length(indexes_v)]]), 0)
  # 
  # for(i in indexes_v){
  ...
  #   index_n <- indexes[[1]][4]
  #   
  #   var_v$i[id] <- as.numeric(stringr::str_sub(model$n_variable[i], start = index_i+1, end = index_k-1)) + 1
  #   var_v$k[id] <- as.numeric(stringr::str_sub(model$n_variable[i], start = index_k+1, end = index_t-1)) + 1
  #   var_v$t[id] <- as.numeric(stringr::str_sub(model$n_variable[i], start = index_t+1, end = index_n-1)) + 1
  #   var_v$n[id] <- as.numeric(stringr::str_sub(model$n_variable[i], start = index_n+1, end = stringr::str_length(model$n_variable[i])))
  # }
  ...
  #                     i = rep(0, size_y),
  #                     t = rep(0, size_y),
  #                     sol = solution$x[indexes_y[1]:indexes_y[length(indexes_y)]])
  #
  # for(i in indexes_y){
